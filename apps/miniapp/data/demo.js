const activities = [
  {
    ...
    suitedFor: ['想暂时离开忙乱，安静照顾自己', '愿意感受身体，不追求标准答案'],
    talkFirst: ['近期有明显或持续不适', '对饮食安排有特殊医疗要求'],
    rules: {
      change: '如需改期，请在活动开始前联系工作人员；具体安排以报名确认信息为准。',
      body: '现场不替代医疗评估。出现明显不适时可以随时暂停，并告知导游。',
      privacy: '个人记录默认仅自己可见；匿名分享会再次征得你的同意。'
    },
    days: [
  ...
  },
  {
    ...
    suitedFor: ['想为秋天留出三天慢下来的时间', '希望重新感受饮食与日常节奏'],
    talkFirst: ['近期有明显或持续不适', '无法连续参加三天现场活动'],
    rules: {
      change: '开放报名后将同步完整的改期与取消规则。',
      body: '现场不替代医疗评估。出现明显不适时可以随时暂停。',
      privacy: '个人记录默认仅自己可见；匿名分享会再次征得你的同意。'
    },
    days: [
      { id: 'day-1', label: '第1天', time: '10:00—17:00', theme: '慢下来' },
...
const uiTestScenarios = {
  normal: { activities, articles, posts: seedPosts, participations },
  empty: { activities: [], articles: [], posts: [], participations: [] },
  failed: { loading: false, error: '网络暂时没有回应，请稍后再试。' }
}

const activityStatusFixtures = [
  { status: '开放报名', tone: 'open' },
  { status: '即将满员', tone: 'warning' },
  { status: '已满', tone: 'full' },
  { status: '候补中', tone: 'waitlist' },
  { status: '已取消', tone: 'cancelled' }
]
