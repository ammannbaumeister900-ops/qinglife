...
async function image(id) {
  ...
  return new Promise((resolve, reject) => wx.downloadFile({ url: base + '/app/qinglife/staff/images/' + encodeURIComponent(id), header: { token }, success: r => r.statusCode === 200 ? resolve(r.tempFilePath) : reject(new Error('图片读取失败或权限已失效')), fail: reject }))
}
