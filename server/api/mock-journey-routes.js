...
module.exports = function journeyRoutes({ customers, sessions, registrations, attendance, send, readBody }) {
  ...
  const registrationView = (r, me) => {
    ...
    return { registrationId: r.id, customerId: r.customerId, participantName: person.nickname, isSelf: r.customerId === me.id, registrationStatus: r.registrationStatus, paymentStatus: r.paymentStatus, sessionId: s.id, sessionName: s.name, sessionNumber: s.sessionNumber, sessionStatus: s.status, startDate: s.startDate, endDate: s.endDate, batchId: r.batchId, relation: r.relation || '本人' }
  }
  return async function handle(req, res, pathname) {
    ...
    try {
      ...
      if (pathname === '/app/qinglife/registrations' && req.method === 'POST') {
        ...
        if (!body.serviceConsent || !Array.isArray(body.participants) || !body.participants.length || body.participants.length > 10) fail('请确认参与人和服务必要授权')
        ...
        const people = body.participants.map(p => {
          if (!p.name || (!p.minor && !/^1\d{10}$/.test(p.phone || ''))) fail('请填写参与人姓名及成年人本人手机号')
        ...
        const batchId = randomUUID(), result = { id: batchId, orderNo: 'TEST-' + batchId.slice(0,8), registrationStatus: s.registrationConfirmMode === 'auto' ? 'confirmed' : 'pending', paymentStatus: 'unpaid', registrations: [] }
        for (const { person, input } of people) {
          ...
          const r = { id: randomUUID(), sessionId: s.id, customerId: person.id, buyerCustomerId: me.id, batchId, orderNo: result.orderNo, registrationStatus: result.registrationStatus, paymentStatus: 'unpaid', sessionReferrerCustomerId: referrer, registrationSource: 'mini_program', relation: input.relation || '本人', revision: 0, registeredAt: stamp() }
      ...
      if (pathname === '/app/qinglife/experience-records' && req.method === 'PUT') {
        ...
        if (body.phase === 'before' && today() >= s.startDate) fail('活动前记录已结束')
        if (body.phase === 'during' && !days(s).some(day => day.id === body.sessionDayId && day.activityDate === today())) fail('只能保存当天活动感受')
        if (body.phase === 'after' && s.status !== 'completed' && today() <= s.endDate) fail('活动尚未结束')
