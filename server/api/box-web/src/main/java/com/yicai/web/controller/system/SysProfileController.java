...
import com.yicai.common.utils.file.FileUploadUtilsbak;
...
public class SysProfileController extends BaseController
{
    ...
    public AjaxResult profile()
    {
        ...
        user.setAvatar(RuoYiConfig.getProfileImage() + user.getAvatar());
		ajax.put("user", user);
    ...
    public AjaxResult avatar(@RequestParam("avatarfile") MultipartFile file) throws IOException
    {
        if (!file.isEmpty())
        {
            ...
            {
				Map<String,Object> ajax = new HashMap<>();
                ajax.put("imgUrl", RuoYiConfig.getProfileImage() + avatar);
                // 更新缓存用户头像
                loginUser.getUser().setAvatar(RuoYiConfig.getProfileImage() + avatar);
                tokenService.setLoginUser(loginUser);
                return AjaxResult.success(ajax);
            }
        }
