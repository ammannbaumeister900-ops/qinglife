...
public class QlStaffWorkspaceService {
    ...
    public List<Map<String,Object>> registrations(String sessionId,String customerId){return db.queryForList("SELECT r.id,r.customer_id AS customerId,r.session_id AS sessionId,r.batch_id AS batchId,r.registration_status AS registrationStatus,r.payment_status AS paymentStatus,c.nickname,c.real_name AS realName,s.session_number AS sessionNumber,s.name AS sessionName,s.start_date AS startDate,s.end_date AS endDate,s.status AS sessionStatus,COALESCE(b.submitted_by_customer_id,r.customer_id) AS contactCustomerId,b.contact_name AS contactName,b.contact_phone AS contactPhone,COALESCE(b.quoted_amount,r.unit_price) AS quotedAmount,COALESCE(b.final_amount,r.final_amount) AS finalAmount,COALESCE(b.settlement_status,r.settlement_status) AS settlementStatus,COALESCE(b.settlement_type,r.settlement_type) AS settlementType,COALESCE(b.pass_units,r.pass_units) AS passUnits FROM ql_registration r JOIN ql_customer c ON c.id=r.customer_id JOIN ql_session s ON s.id=r.session_id LEFT JOIN ql_registration_batch b ON b.id=r.batch_id WHERE "+(sessionId!=null?"r.session_id=?":"r.customer_id=?")+" ORDER BY s.start_date DESC,r.created_at DESC",sessionId!=null?sessionId:customerId);}
    public List<Map<String,Object>> passes(String customerId){return db.queryForList("SELECT a.id,a.pass_type AS passType,a.status,a.valid_until AS validUntil,COALESCE(SUM(l.quantity_delta),0) AS balance,CASE WHEN a.status='active' AND (a.valid_from IS NULL OR a.valid_from<=CURRENT_DATE()) AND (a.valid_until IS NULL OR a.valid_until>=CURRENT_DATE()) THEN 1 ELSE 0 END AS usable FROM ql_pass_account a LEFT JOIN ql_pass_ledger l ON l.pass_account_id=a.id WHERE a.customer_id=? GROUP BY a.id,a.pass_type,a.status,a.valid_from,a.valid_until",customerId);}
